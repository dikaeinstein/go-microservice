...
func BenchmarkGeneratePlainHash(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		_, _, _ = h.GenerateHash(original, false, false)
	}
}
...
func BenchmarkGenerateHashWithSaltAndPepper(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		_, _, _ = h.GenerateHash(original, true, true)
	}
}
...
func benchmarkCompare(pepperSize int, b *testing.B) {
	...
	h := New(p)
	hash, salt, _ := h.GenerateHash(original, true, true)

	for i := 0; i < b.N; i++ {
		h.Compare(original, salt, hash, true)
	}
}

func BenchmarkCompareSaltedHash(b *testing.B) {
	h := New(peppers)
	hash, salt, _ := h.GenerateHash(original, true, false)

	for i := 0; i < b.N; i++ {
		h.Compare(original, salt, hash, false)
	}
}

func BenchmarkComparePlainHash(b *testing.B) {
	h := New(peppers)
	hash, _, _ := h.GenerateHash(original, false, false)

	for i := 0; i < b.N; i++ {
		h.Compare(original, "", hash, false)
	}
}
...
func TestCompareSaltedAndPeppered(t *testing.T) {
	h := New(peppers)
	success, _ := h.Compare(original, saltedAndPepperedSalt, saltedAndPepperedHash, true)
...
func TestComparesSaltedCorrectly(t *testing.T) {
	h := New(peppers)
	success, _ := h.Compare(original, saltedSalt, saltedHash, false)
...
func TestComparesPlainCorrectly(t *testing.T) {
	h := New(peppers)
	success, _ := h.Compare(original, "", plainHash, false)
